...
const contactInfo = [
  { id: 1, label: "Phone", value: "+91 99155 89863", icon: "fas fa-envelope" },
  {
    id: 2,
    label: "Mail",
    value: "business@noctu.tech",
...
const Contact = () => {
  ...
  const onSubmit = (e) => {
    ...
    if (
      ...
      const mailtoLink = `mailto:business@noctu.tech?subject=${encodeURIComponent(
        subject
      )}&body=${encodeURIComponent(
        `Name: ${name}\nEmail: ${email}\n\nMessage:\n${message}`
      )}`;

      // Open the mailto link (this will open Gmail or the default email client)
